...
export default function SignUp() {
  ...
  useEffect(() => {
    axAuth(token)({
      ...
      // 성공했을 시, then 이하 실행 / catch는 실패했을 시
      const data = res.data;
      setMember_id(data);
      setUserData(data);
    });
      ...
      <div className="mt-10">
        ...
        <article className="px-[4.5%]">
          <div className="mypage-content-one">
            ...
            <div className="text-black font-bold text-2xl mt-[0.3rem] w-[4.5rem] h-[1.6rem] ml-6">{userData.result.userName}</div>
            <div className="text-dark-grey font-bold mt-[0.1rem] text-l w-[3rem] ml-8">{userData.result.userStudentId}</div>
          </div>
          <div>
            <div className="grid-cols-1 devide-y mt-[3rem] bg-blue border-grey border text-xl rounded-lg">
              ...
              <a onClick={() => router.push('')}>
                <MyPageButton text={'내가 쓴 댓글 보기'} addClass="text-xl mt-0" />
              </a>
              <a onClick={() => router.push('')}>
                <MyPageButton text={'나의 출석 현황'} addClass="text-xl mt-0" />
              </a>
              <a onClick={() => router.push('')}>
                <MyPageButton text={'휴면 전환하기'} addClass="text-xl mt-0" />
              </a>
              <a onClick={() => router.push('')}>
                <MyPageButton text={'동아리 탈퇴하기'} addClass="text-xl mt-0" />
              </a>
            </div>
          </div>
        </article>
