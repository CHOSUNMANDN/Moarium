...
public class ClubDetailPageController {
  ...
  public BaseResultDTO<DormancysMembersResDto> getDormancysMemberPage(HttpServletRequest request,
      @PathVariable(value = "club_id") Long clubId) {
    List<String> allDormancyMemberNamesOfClub = clubDetailPageService.getAllDormancyMemberNamesOfClub(
        clubId);
  ...
  @GetMapping(value = "")
  public BaseResultDTO<ClubMemberDetailResDto> getMemberDetailsPage(HttpServletRequest request,
      @PathVariable(value = "club_id") Long clubId) {
    ...
    // 동아리 회원의 상세 페이지 조회
    ClubMemberDetailResDto clubMemberResponse = clubDetailPageService.getClubMemberResponse(clubId,
        memberId, myClubGrade.getClubGrade());
  ...
  public BaseResultDTO<ClubMemberInformationResDto> getClubMemberDetails(
    ...
    ClubMemberInformationResDto clubMemberResponse = clubDetailPageService.getClubMemberInformationResDto(
        clubMemberId);
  ...
  public ResponseEntity<BaseResponseBody> giveVacationTokenToClubMember(
    ...
    return BaseResponseBodyUtiil.BaseResponseBodyFailure(
        RESPONSE_MESSAGE_VALUE.FAIL_UPDATE_VACATION_TOKEN(targetMember.getFullName()));
  }
  ...
  public ResponseEntity<BaseResponseBody> dormancyClubMember(
    ...
    // 휴면 등급 업데이트
    Boolean updateSuccess = clubGradeService.updateClubGradeOfClubMember(clubMemberId,
        CLUB_GRADE.DORMANT);
    if (updateSuccess) {
      return BaseResponseBodyUtiil.BaseResponseBodySuccess(
    ...
    return BaseResponseBodyUtiil.BaseResponseBodyFailure(
        RESPONSE_MESSAGE_VALUE.FAIL_UPDATE_CLUB_GRADE(targetMember.getFullName()));
  }
  ...
  public ResponseEntity<BaseResponseBody> checkAttendanceNumber(
    ...
    Boolean isPassTodayAttendance = attendanceNumberService.checkAttendanceNumber(clubId,
        attendanceNumberReqDto.getNumOfAttendance());
    Boolean isPossibleAttendance = attendanceWeekDateService.isPossibleUpdateAttendanceState(clubId,
        memberId);

    // 클럽 지정 출석을 체크
    String clubPossibleStatus = attendanceWeekService.getTodayPossibleStatus(clubId,
        DayUtil.getTodayDayOfWeek());
    if (POSSIBLE_STATUS.POSSIBLE.isNot(clubPossibleStatus)) {
      return BaseResponseBodyUtiil.BaseResponseBodyFailure(
          ResponseMessage.BAD_REQUEST_NOT_CLUB_CHECK_STATE.get());
    }

    // 로그인 한 회원이 타겟의 유저인지 확인
    Boolean isSameMemberTargetAndLogin = memberService.isSameTargetAndLoginMember(memberId,
        clubMemberId);
    ...
    if (isDormantMember) {
      return BaseResponseBodyUtiil.BaseResponseBodyFailure(
          ResponseMessage.BAD_DORMANT_TODAY_ATTENDANCE_STATE.get());
    }
    ...
    if (isPassTodayAttendance) {
      // 출석 상태 전환
      attendanceWeekDateService.updateTodayAttendanceWeekDate(clubMemberId,
          ATTENDANCE_STATE.ATTENDANCE);
      return BaseResponseBodyUtiil.BaseResponseBodySuccess(
          ResponseMessage.PASS_TODAY_ATTENDANCE.get());
    }

    return BaseResponseBodyUtiil.BaseResponseBodyFailure(
        ResponseMessage.BAD_TODAY_ATTENDANCE.get());
  }
}